...
fn restart_engines(fx: &EngineFixture, tag_a: &str, tag_b: &str) -> (EngineHandle, EngineHandle) {
    ...
    let mut engine_a = SyncEngine::new(cfg_a, default_transport()).expect("engine A restart");
    // Same explicit TOFU opt-in as the fixture that paired these stores.
    engine_a.set_peer_policy(engine::PeerPolicy::TrustOnFirstUse);
    let addr = engine_a
        .listen_addr()
    ...
    let mut engine_b = SyncEngine::new(cfg_b, default_transport()).expect("engine B restart");
    engine_b.set_peer_policy(engine::PeerPolicy::TrustOnFirstUse);
