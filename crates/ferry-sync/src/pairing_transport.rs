...
impl PairingTransport {
    ...
    fn find_folder_path(&self, folder_id_hex: &str) -> Option<PathBuf> {
        ...
        // Try daemon registry at $FERRY_HOME/folders.toml (single owner:
        // ferry-folder's inventory module).
        if let Ok(records) = ferry_folder::inventory::FolderInventory::new(&self.home).list() {
            for rec in records {
                if rec.folder_id.to_ascii_lowercase() == key
                    || rec
                        .folder_id
                        .to_ascii_lowercase()
                        .starts_with(&key[..key.len().min(32)])
                {
    ...
    pub fn create_session(&self, folder_id: String) -> Result<CreatePairingResponse, OpError> {
        ...
        let folder_path = self.find_folder_path(&folder_id_hex).ok_or_else(|| {
            OpError::new(
                "not-found",
        ...
        let opened = open_folder(&folder_path, &self.identity)
        ...
        let entry = head
            .entries
            .iter()
            .find(|e| e.device_pub == *self.identity.public())
            .ok_or_else(|| {
                OpError::new(
        ...
        let fmk_z =
            unwrap_folder_key(&entry.wrapped, &head.folder_id, &self.identity).map_err(|e| {
                OpError::new(
                    "key-unwrap",
                    e.to_string(),
    ...
    pub fn join_session(&self, req: JoinPairingRequest) -> Result<PairResult, OpError> {
        ...
        let wrapped_for_peer =
            wrap_folder_key(&session.fmk, &session.folder_id, self.identity.public()).map_err(
                |e| {
                    OpError::new(
                        "crypto",
                        e.to_string(),
        ...
        let sealed = seal_pair_grant(&session.offer_bytes, &body)
            .map_err(|e| OpError::new("crypto", format!("grant seal failed: {e}"), "retry"))?;

        // B opens grant.
        let opened_body = open_pair_grant(&session.offer_bytes, &sealed).map_err(|e| {
            OpError::new(
                "bad-grant",
                format!("grant open failed: {e}"),
                "redo the pairing",
            )
        })?;
        let doc: serde_json::Value = serde_json::from_slice(&opened_body)
            .map_err(|_| OpError::new("bad-grant", "grant body unreadable", "redo the pairing"))?;
        ...
        let fmk_unwrapped = *unwrap_folder_key(&wrapped, &session.folder_id, &self.identity)
            .map_err(|e| {
                OpError::new(
                    "key-unwrap",
                    e.to_string(),
        ...
        let _ = store
            .flush()
            .map_err(|e| OpError::new("store", e.to_string(), "retry"));
        let _ = store
        ...
        let wrapped_for_initiator =
            wrap_folder_key(&fmk_unwrapped, &session.folder_id, &session.initiator_pub).map_err(
                |e| {
                    OpError::new(
                        "crypto",
                        e.to_string(),
...
fn append_wrap_entry(
    root: &Path,
    folder_id: [u8; 16],
    recipient: &[u8; 32],
    wrapped: &[u8; WRAPPED_LEN],
) -> Result<(), ferry_folder::FolderError> {
    let path = dot_dir(root).join(CONFIG_FILE);
    let bytes = std::fs::read(&path).map_err(|_| {
        ferry_folder::FolderError::new(
            "config-corrupt",
            "missing key envelope",
            "restore from backup or re-pair",
        )
    })?;
    let head = parse_config_head(&bytes).map_err(|_| {
        ferry_folder::FolderError::new(
            "config-corrupt",
            "restore from backup",
            "restore from backup",
    ...
    std::fs::write(&tmp, &updated).map_err(|e| {
        ferry_folder::FolderError::new(
            "io",
    ...
    std::fs::rename(&tmp, &path).map_err(|e| {
        ferry_folder::FolderError::new(
            "io",
            format!("cannot finalize {}", path.display()),
            e.to_string(),
        )
    })?;
    Ok(())
