...
pub fn render_conflicts_modal(
    ...
    render_modal_frame(ctx, "Quarantined Conflicts Inspection", is_open, 560.0, |ui, _open| {
        ui.horizontal(|ui| {
            ui.label(
                RichText::new(format!("Total Unresolved Conflicts: {}", conflicts.len()))
                    .color(if conflicts.is_empty() { colors::FERRY_GREEN } else { colors::RED_CONFLICT })
                    .strong(),
            );
            ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
            ...
            Frame::none()
                ...
                .show(ui, |ui| {
                    ui.vertical_centered(|ui| {
                        ui.label(RichText::new("✓ No Quarantined Conflicts").color(colors::FERRY_GREEN).strong().size(14.0));
                        ui.add_space(4.0);
                        ui.label(RichText::new("All local and remote file states are clean and reconciled.").color(colors::TEXT_MUTED).size(12.0));
                    });
                });
        } else {
            ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    for (idx, c) in conflicts.iter().enumerate() {
                        Frame::none()
                            ...
                            .show(ui, |ui| {
                                ui.horizontal(|ui| {
                                    ui.label(RichText::new(format!("#{idx} • {}", c.ts)).color(colors::TEXT_MUTED).size(11.0));
                                    ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                                        ui.monospace(RichText::new(format!("kind: {}", c.kind)).color(colors::AMBER_WARN).size(11.0));
                                    });
                                });

                                ui.add_space(4.0);
                                ui.label(RichText::new(&c.path).color(colors::RED_CONFLICT).strong().size(13.0));

                                if let Some(ref q) = c.quarantined_as {
                                    ui.add_space(2.0);
                                    ui.horizontal(|ui| {
                                        ui.label(RichText::new("Quarantined copy:").color(colors::TEXT_MUTED).size(11.5));
                                        ui.monospace(RichText::new(q).color(colors::TEXT_SECONDARY).size(11.5));
                                    });
                                }

                                ui.add_space(2.0);
                                ui.horizontal(|ui| {
                                    ui.label(RichText::new("Winning device:").color(colors::TEXT_MUTED).size(11.5));
                                    ui.monospace(RichText::new(format_short_hex(Some(&c.winner.device))).color(colors::FERRY_GREEN).size(11.5));
                                    ui.label(RichText::new("│").color(colors::GLASS_BORDER));
                                    ui.label(RichText::new("Losing device:").color(colors::TEXT_MUTED).size(11.5));
                                    ui.monospace(RichText::new(format_short_hex(Some(&c.loser.device))).color(colors::RED_CONFLICT).size(11.5));
                                });
                            });
                        ui.add_space(6.0);
                    }
...
pub fn render_share_modal(
    ...
    render_modal_frame(ctx, "Share Folder & Pairing Ritual", is_open, 520.0, |ui, open| {
        if let Some(offer) = active_offer {
            ...
            Frame::none()
                ...
                .inner_margin(Margin::same(12.0f32))
                .show(ui, |ui| {
                    ui.label(RichText::new("32-Character Pairing Short Code:").color(colors::TEXT_MUTED).size(11.0));
                    ui.horizontal(|ui| {
                        ui.monospace(RichText::new(&offer.token).color(colors::FERRY_GREEN).strong().size(13.0));
                        ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                            if ui.button(RichText::new("📋 Copy Token").size(12.0)).clicked() {
                                ui.ctx().output_mut(|o| o.copied_text.clone_from(&offer.token));
                            }
                        });
                    });
                    ...
                    if let Some(ref path) = offer.payload_path {
                        ui.add_space(4.0);
                        ui.horizontal(|ui| {
                            ui.label(RichText::new("Payload file:").color(colors::TEXT_MUTED).size(11.0));
                            ui.monospace(RichText::new(path.display().to_string()).color(colors::TEXT_SECONDARY).size(11.0));
                        });
                    }
                });
            ...
            Frame::none()
                ...
                .show(ui, |ui| {
                    ui.vertical_centered(|ui| {
                        ui.label(RichText::new("QR Code Representation").color(colors::TEXT_MUTED).size(11.0));
                        ui.add_space(4.0);
                        ScrollArea::both()
                            .max_height(140.0)
                            .show(ui, |ui| {
                                ui.monospace(RichText::new(qr_string).color(Color32::WHITE).size(8.0));
                            });
                    });
                });
            ...
            ui.add_space(10.0);
            ui.horizontal(|ui| {
                if ui.button(RichText::new("Done").color(colors::FERRY_GREEN)).clicked() {
                    *open = false;
                }
            });
        } else {
            ...
            ui.horizontal(|ui| {
                ...
                .fill(if can_generate { colors::FERRY_GREEN } else { colors::PANEL_BG });
...
pub fn render_pair_modal(
    ...
    render_modal_frame(ctx, "Pair Device (Accept Offer)", is_open, 480.0, |ui, open| {
        ui.label(RichText::new("Enter the filesystem path to the incoming `.ferry-pair` offer payload file:").color(colors::TEXT_SECONDARY).size(12.5));
        ui.add_space(6.0);

        ui.text_edit_singleline(pair_input);
        ui.label(RichText::new("e.g. /home/user/downloads/pair-offer.ferry-pair").color(colors::TEXT_MUTED).size(11.0));
        ...
        ui.horizontal(|ui| {
            ...
            let btn = egui::Button::new(
                RichText::new("Accept Pairing Offer")
                    .color(if is_valid { Color32::BLACK } else { colors::TEXT_MUTED })
                    .strong(),
            )
            .fill(if is_valid { colors::FERRY_GREEN } else { colors::PANEL_BG });
...
pub fn render_pin_modal(
    ...
    render_modal_frame(ctx, "Session Pinning (Hold Edits)", is_open, 480.0, |ui, open| {
        ...
        ui.horizontal(|ui| {
            if ui.button(RichText::new("Start Session Pin (8h)").color(Color32::BLACK).strong()).clicked() {
