# Ticket 08: Fix Convergence Race Conditions and Peer-Policy Sync Failures
...
- [x] `cargo test -p ferry-cli --test exchange_loopback` passes consistently in < 10s.
- [x] `cargo test -p ferry-cli --test ui_server_tests` passes cleanly.
- [x] `cargo test -p ferry-scan` passes with 0 regressions.
- [x] `cargo test -p ferry-sync-engine` passes with 0 regressions.
- [x] Uncommitted changes in working tree are committed cleanly.

## Comments

Pulled from ponytail-launch-cleanup Ticket 12 into live-testing-remediation spec.
