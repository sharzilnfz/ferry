...
## Acceptance Criteria
- [x] `ferry ui` starts Axum on a random loopback port and opens the browser.
- [x] Requests without the valid one-time token are rejected with HTTP 403 Forbidden.
- [x] Web dashboard endpoints (`/api/status`, `/api/conflicts`) fetch live data from the daemon over IPC.
- [x] Server shuts down automatically after 10 minutes of inactivity or on `Ctrl+C`.
